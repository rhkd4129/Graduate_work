...
################ <---- 세번째 줄 ---->  ###############
    ...
    GObtn = Button(graduate_App,text='Search')
    GObtn.place(x = 400, y=250)
    GObtn.config(command = Search_Button)
...
def duplication_screen(search_image_name,number_entry_num)->bool:
    ...
    for i,duplication_word_btn in enumerate(duplication_words):
        ...
        text.append(btn_dict[duplication_word_btn]['text'])
        x+=200  
    
    # TODO:
    # 조금의 하드 코딩 
    if len(text) ==2:
        btn_dict[text[0]].config(command = lambda : duplication_screen_crawing_execute(text[0],number_entry_num))
        btn_dict[text[1]].config(command = lambda : duplication_screen_crawing_execute(text[1],number_entry_num))
    elif len(text) ==3:
        btn_dict[text[0]].config(command = lambda : duplication_screen_crawing_execute(text[0],number_entry_num))
        btn_dict[text[1]].config(command = lambda : duplication_screen_crawing_execute(text[1],number_entry_num))
        btn_dict[text[2]].config(command = lambda : duplication_screen_crawing_execute(text[2],number_entry_num))
        
    graduate_App.option_add("*Font","맑은고딕 17")
